...
class LunarCalendar {
  ...
  static getDayCanChi(day, month, year) {
  ...
  static getAuspiciousHoursForDay(day, month, year, language = 'vi') {
    const { branch } = this.getDayCanChi(day, month, year);
    const goodBranches = GOOD_HOURS_BY_DAY_BRANCH[branch] || [];
    const formatLabel = (hourBranch) => {
      const info = HOUR_RANGES[hourBranch];
      return {
        branch: hourBranch,
        label: language === 'vi' ? info.vi : info.en,
        start: info.start,
        end: info.end
      };
    };
